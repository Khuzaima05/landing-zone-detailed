...
This is the simplest way to create multiple namespaces with labels and annotations.
...
Each entry in `namespaces` becomes a namespace definition. The most important fields are:

- `name`: the Kubernetes namespace name
- `labels`: searchable key-value metadata
- `annotations`: descriptive or automation-oriented metadata

For beginners, it helps to think of the module input as a high-level declaration of desired cluster workspaces.
...
This tells Terraform to create the namespace and then attach a resource governance policy to it.
...
This is useful when you want workloads to follow default resource sizing rules even if application teams forget to set them explicitly.
...
This configuration creates access rules around the namespace so users and automation accounts only get the permissions they need.
...
This pattern is common in landing zones because it combines ownership, environment separation, and resource governance in one Terraform definition.
