...
For beginners, this can feel abstract. This document makes the translation explicit.
...
- `labels` become `metadata.labels`
- `annotations` become `metadata.annotations`

This is the simplest and most direct translation in the module.
...
- the key `production` identifies the target namespace
- the `hard` map becomes `spec.hard`
- the `ResourceQuota` object is created inside the `production` namespace

This means Terraform is defining a namespace budget that Kubernetes will enforce.
...
- the map key `production` selects the namespace
- each Terraform `limits` item becomes one entry in `spec.limits`
- Terraform `default_request` maps conceptually to Kubernetes `defaultRequest`

This object sets sizing rules that apply to containers or pods in that namespace.
...
- `namespace` becomes `metadata.namespace`
- each item in `subjects` becomes one entry in `subjects`
- `role_name` becomes `roleRef.name`
- common built-in roles such as `view`, `edit`, and `admin` are often referenced as cluster roles

Even though the role reference may point to a `ClusterRole`, the binding itself is namespace-scoped because it is a `RoleBinding` in one namespace.
...
- this binding is cluster-scoped, not namespace-scoped
- use it only when broad access is actually required
...
- `namespace` controls where the service account lives
- `name` becomes `metadata.name`
- `annotations` become `metadata.annotations`

This identity can then be referenced by pods or additional RBAC bindings.
...
- the map key `backend` usually identifies the namespace where the policy is created
- `pod_selector` maps to `spec.podSelector`
- `policy_types` maps to `spec.policyTypes`
- ingress and egress blocks translate to rule lists in YAML
